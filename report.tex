...
\chapter{Задание 4: Реализация веб-интерфейса на CherryPy}

\section{Пояснение к выполнению Задания 4}

В соответствии с источниками (пример \texttt{example5/start.py} и \texttt{example5/bookpage.py}), четвёртый этап разработки заключается в создании веб-интерфейса на базе микро-фреймворка CherryPy. Веб-интерфейс предоставляет альтернативный способ взаимодействия с данными через браузер, сохраняя при этом ту же объектную модель и слой хранения данных.

Архитектура веб-интерфейса повторяет шаблон, продемонстрированный в учебном примере \texttt{example5}:

\begin{enumerate}
    \item \textbf{Точка входа (\texttt{start.py}):} главный класс CherryPy-приложения. В методе \texttt{index()} отображает форму открытия файла (до загрузки данных) или навигационные ссылки на страницы сущностей и форму сохранения файла (после загрузки).
    \item \textbf{Страницы сущностей:} отдельные классы (\texttt{departmentpage}, \texttt{expensetypepage}, \texttt{expensepage}) для каждой сущности. Каждый класс содержит методы для отображения списка (\texttt{index}), добавления (\texttt{addform}, \texttt{addaction}), редактирования (\texttt{editform}, \texttt{editaction}) и удаления (\texttt{delr}) записей.
    \item \textbf{Генерация HTML:} страницы формируются как строки HTML непосредственно в коде Python (без шаблонизаторов), с использованием конкатенации строк и форматирования через оператор \%.
    \item \textbf{Маршрутизация:} CherryPy автоматически сопоставляет URL-пути с методами классов-страниц, имеющими атрибут \texttt{exposed=True}.
\end{enumerate}

\subsection{Модуль \texttt{start.py} --- точка входа веб-приложения}

Главный класс \texttt{start} инициализирует объект \texttt{accounting}, объект \texttt{dataxml} и три страницы сущностей как атрибуты. CherryPy автоматически монтирует их как подпути URL: \texttt{/departmentpage}, \texttt{/expensetypepage}, \texttt{/expensepage}.

\lstinputlisting[label=lst:start,caption=Модуль \texttt{start.py} --- точка входа CherryPy веб-приложения]{start.py}

\subsection{Модуль \texttt{departmentpage\_web.py} --- страница отделов}

Класс \texttt{departmentpage} предоставляет веб-интерфейс для управления отделами. Метод \texttt{index()} генерирует HTML-таблицу со списком всех отделов и ссылками на редактирование и удаление. Методы \texttt{addform()} и \texttt{editform()} отображают HTML-формы, а \texttt{addaction()} и \texttt{editaction()} обрабатывают POST-запросы.

\lstinputlisting[label=lst:departmentpageweb,caption=Модуль \texttt{departmentpage\_web.py} --- CherryPy страница отделов]{departmentpage_web.py}

\subsection{Модуль \texttt{expensetypepage\_web.py} --- страница видов расходов}

Класс \texttt{expensetypepage} предоставляет аналогичный веб-интерфейс для видов расходов с полями: название, описание и предельная норма.

\lstinputlisting[label=lst:expensetypepageweb,caption=Модуль \texttt{expensetypepage\_web.py} --- CherryPy страница видов расходов]{expensetypepage_web.py}

\subsection{Модуль \texttt{expensepage\_web.py} --- страница расходов}

Класс \texttt{expensepage} реализует веб-интерфейс для расходов. Форма редактирования содержит выпадающие списки (HTML \texttt{<select>}) для выбора вида расхода и отдела, генерируемые методами \texttt{expensetypeCombo()} и \texttt{departmentCombo()}. Это соответствует шаблону \texttt{publCombo()} из примера \texttt{example5/bookpage.py}.

\lstinputlisting[label=lst:expensepageweb,caption=Модуль \texttt{expensepage\_web.py} --- CherryPy страница расходов]{expensepage_web.py}

\section{Запуск веб-приложения}

Веб-приложение запускается командой:

\begin{verbatim}
python start.py
\end{verbatim}

После запуска CherryPy выводит в консоль URL (по умолчанию \texttt{http://127.0.0.1:8080}). Пользователь открывает этот адрес в браузере и видит:

\begin{enumerate}
    \item \textbf{Начальный экран:} форма для открытия файла --- поле имени файла и выпадающий список формата (XML).
    \item \textbf{После загрузки:} навигационные ссылки на страницы «Отделы», «Виды расходов», «Расходы», а также форма для сохранения данных в файл.
\end{enumerate}

\section{Скриншоты веб-интерфейса}

\subsection{Форма открытия файла (начальный экран)}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{screenshots/cherrypy_upload_gui.png}
\caption{Начальный экран CherryPy --- форма открытия файла}
\label{fig:scr_cherrypy_open}
\end{figure}

На рисунке~\ref{fig:scr_cherrypy_open} показан начальный экран веб-приложения. Пользователь вводит имя файла (например, \texttt{data.xml}), выбирает формат XML и нажимает кнопку отправки. Серверная часть через \texttt{dataxml.readFile()} загружает данные в объект \texttt{accounting}.

\subsection{Список расходов после загрузки данных}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{screenshots/cherrypy_result_gui.png}
\caption{Страница «Расходы» веб-интерфейса после загрузки \texttt{data.xml}}
\label{fig:scr_cherrypy_result}
\end{figure}

На рисунке~\ref{fig:scr_cherrypy_result} показана страница списка расходов после загрузки данных из XML-файла. Таблица отображает все четыре расхода с информацией о виде расхода, отделе, сумме и дате. В каждой строке присутствуют ссылки «редактировать» и «удалить». Ссылка «назад» вверху страницы возвращает на главную страницу, а ссылка «добавить» открывает форму создания нового расхода.

\chapter{Заключение}

В результате выполнения курсовой работы разработано приложение на языке Python с графическим интерфейсом PyQt5 и веб-интерфейсом CherryPy для учёта внутриофисных расходов. Приложение реализует:
...
\begin{itemize}
    ...
    \item Полноценный графический интерфейс PyQt5 с таблицами, формами редактирования, выпадающими списками для выбора связанных объектов и календарём для выбора даты.
    \item Веб-интерфейс на CherryPy, позволяющий работать с данными через браузер: загружать XML-файлы, просматривать списки, добавлять, редактировать и удалять записи, сохранять данные обратно в XML.
    \item Поддержку сохранения и загрузки данных в формате XML с использованием библиотеки \texttt{xml.dom.minidom}.
    \item Архитектуру, полностью соответствующую шаблонам, представленным в учебных примерах \texttt{example4} (PyQt5 GUI) и \texttt{example5} (CherryPy web).
\end{itemize}

Приложение готово к использованию в бухгалтерии частной фирмы для отслеживания внутриофисных расходов сотрудников как в настольном режиме (PyQt5), так и через браузер (CherryPy).

\vspace{10pt}
Общий объём исходного кода составляет 34 файла (около 1000 строк), что демонстрирует масштабируемость выбранной архитектуры и её пригодность для дальнейшего расширения (добавление поддержки JSON, SQLite и других форматов хранения).
